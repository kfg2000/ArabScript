...
const semanticChecks = [
  ...
  ["Try Catch", "{ دع ا = ١؛ } مسك(ب){ طبع(ب)؛ }"],
  ["Class Definition", "صنف كلب { }"],
  [
  ...
  ["Function Definition", "دالة بارك(){}"],
  ["Function Definition with params", "دالة بارك(ا،ب){}"],
  ["Function Definition with empty return", "دالة بارك(ا،ب){ عد؛ }"],
  ["Function Definition with return", " دالة بارك(ا،ب){ عد ا؛ }"],
  ["Function call", " دالة بارك(ا،ب){ عد ا؛ } بارك(١،٢)؛"],
  ["If statement", 'دع ط = ١**٢-٣٩/٢٣؛ متغير اسم = "خالد"؛ لو(ط>٠){ ط=٠؛}'],
  [
    "ِElse If statement",
  ...
  ["While condition with continue", "بينما(!(١>٢)){ استمر؛ }"],
  [
    "Complex While condition with break",
    "متغير بول = صح؛ بينما(!(١>٢)&&بول){ بول = خطا؛ قف؛ }",
  ],
  ["For with args", "ل(دع ه = ١؛ ه>٠؛ه++){ ه += ٣؛ }"],
  ["For of", "دع ه؛ دع ارقام = [١،٢،٣،٤]؛ ل(رقم من ارقام){ ه += رقم؛ }"],
  [
    "Switch Statement",
...
const semanticErrors = [
  ...
  ["unary increment to const", "ثابت ا = ١؛ ا++؛", /Cannot assign to constant ا/],
  ["binary increment to const", "ثابت ا = ١؛ ا+=١؛", /Cannot assign to constant ا/],
  ["unary decrement to const", "ثابت ا = ١؛ ا--؛", /Cannot assign to constant ا/],
  ["binary decrement to const", "ثابت ا = ١؛ ا-=١؛", /Cannot assign to constant ا/],
  ["unary negation to string", 'دع ا = "١"؛ -ا؛', /Expected a number, found string/],
  ["uncallabled variable", "دع اسم = ١؛ اسم()؛", /Call of non-function/],
  [
  ...
  ["non-boolean short if test", "لو(٠){}", /Expected a boolean, found int/],
  ["non-boolean else if test", "لو(صح){}ولو(٠){}", /Expected a boolean, found int/],
  ["non-boolean while test", "بينما(٠){}", /a boolean, found int/],
  ["non-boolean for test", "ل(دع ه = ١؛٠؛ه++){}", /a boolean, found int/],
  ["non-iterable for of test", "دع ارقام = ١؛ ل(رقم من ارقام){}", /Iterable expected/],
  ["non-boolean ternary test", 'متغير اسم = ١ ؟ "خالد" : مجهول؛', /a boolean, found int/],
  ["Object doesnt have unique keys", "دع ا = {١:١، ١:٢}؛", /Keys must be distinct/],
  [
  ...
  ["Property exp for non object", "دع اري = ١؛ اري.ا؛", /Object expected/],
]
