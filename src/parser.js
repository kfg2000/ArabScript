...
const astBuilder = grammar.createSemantics().addOperation("tree", {
  ...
  Statement_varDec(_varType, identifier, _end) {
    return new ast.VariableDec(identifier.tree(), false)
  },
  Statement_multDec(_varType, multDecs, lastDec, _end) {
    return new ast.MultDec([...multDecs.tree(), lastDec.tree()], false)
  },
  Statement_multDecConst(_varType, multDecs, lastDec, _end) {
  ...
  TryCatch(tryBody, _catchKeyword, _left, catchVar, _right, catchBody) {
    return new ast.TryCatch(tryBody.tree(), catchVar.tree(), catchBody.tree())
  },
  ClassDec(_classBeginning, name, _left, constructorBody, body, _right) {
    return new ast.Class(name.tree(), constructorBody.tree(), body.tree())
  },
  This(variable, _thisKeyword) {
    return new ast.This(variable.tree())
  },
  Constructor(_constructorBeginning, _left, params, _right, body) {
    return new ast.Constructor(params.tree(), body.tree())
  },
  FunctionDec(_functionBeginning, name, _left, parameters, _right, body) {
    return new ast.FunctionDec(name.tree(), parameters.tree(), body.tree())
  },
  ...
  ForStatement_forOf(_forBeginning, _left, variable, _of, iterable, _right, body) {
